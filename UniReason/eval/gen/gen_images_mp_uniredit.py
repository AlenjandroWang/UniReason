...
if __name__ == "__main__":
    ...
    metadatas = load_metadatas(args.metadata_file)
    ...
    for local_idx, idx in enumerate(range(start, end), 1):
        ...
        print(f"GPU {rank} processing {local_idx}/{end - start}: '{prompt}' -> {outpath}")

        if os.path.exists(outpath):
            print(f"GPU {rank} skipping (exists): {outpath}")
            continue

        images = [pil_img2rgb(Image.open(img_path))]
