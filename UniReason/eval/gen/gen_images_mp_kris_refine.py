...
def refine_image_with_cot(
    image, src_image,prompt, think_text, num_timesteps=50, 
    cfg_text_scale=4.0, cfg_img_scale=2.0,
    cfg_interval=[0, 1.0], cfg_renorm_min=0., 
    cfg_type="serial_text_img", cfg_renorm_type="text_channel", 
    timestep_shift=3.0, max_image_size=1024, min_image_size=512, img_size=None,
    ...
    cfg_img_newlens = [0]
    cfg_img_new_rope = [0]
    enhance_prompt = f"Strictly apply all thinking_text: {think_text} to refine and correct the previous edit image, and ensure the final edited image is a perfect, faithful, and unambiguous to edit the source image follwing the instruction: {prompt} better than the previous edit image."
    # prepare text list
    text_list = [enhance_prompt]
...
if __name__ == "__main__":
    ...
    os.makedirs(output_dir+"_refine", exist_ok=True)
    if rank == 0:
    ...
    for idx in range(start, end):
        ...
        os.makedirs(os.path.join(output_dir+"_refine", metadata['type']), exist_ok=True)
        outpath = os.path.join(output_dir, metadata['type'], f"{metadata['id']}.png")
        outpath_refine = os.path.join(output_dir+"_refine", metadata['type'], f"{metadata['id']}.png")
        ...
        refine_image.save(outpath_refine)
