...
def refine_image_with_cot(
    ...
    # add question text
    text_list = [SYSTEM_PROMPT_REFINE, prompt,'you must write a brief modification suggestion enclosed in <sugg>...</sugg>']
    ...
    print("="*30, "Refinement", "="*30)
    print(refine_output)

    suggestion = re.findall(r"<sugg>(.*?)</sugg>", refine_output, re.DOTALL)
    ...
    cfg_img_newlens = [0]
    cfg_img_new_rope = [0]
    enhance_prompt = f"Strictly apply all modification suggestions: {refine_output} to refine and correct the input image, and ensure the final edited image is a perfect, faithful, and unambiguous realization of the caption: {prompt} better than the input image."
    # prepare text list
    text_list = [SYSTEM_PROMPT, prompt, enhance_prompt]
    for text_element in text_list:
...
if __name__ == "__main__":
    ...
    for idx in range(start, end):
        ...
        sample_path = f"{output_dir}/{'-'.join(sample_id.split('-')[:2])}"
        sample_path_refine = f"{output_dir}_refine/{'-'.join(sample_id.split('-')[:2])}"
        os.makedirs(f"{output_dir}/{'-'.join(sample_id.split('-')[:2])}", exist_ok=True)
        os.makedirs(f"{output_dir}_refine/{'-'.join(sample_id.split('-')[:2])}", exist_ok=True)

        flag = True
        for idx in range(args.num_images):
            if not os.path.exists(os.path.join(sample_path, f"{sample_id}-{idx}.png")):
                flag = False
                break
            if not os.path.exists(os.path.join(sample_path_refine, f"{sample_id}-{idx}.png")):
                flag = False
                break
        if flag:
        ...
        for i in range(args.num_images // args.batch_size):
            if args.think:
                ...
                think_path = os.path.join(sample_path, f"{sample_id}_think.txt")
        ...
        for sample in image_list:
            ...
            sample.save(os.path.join(sample_path, f"{sample_id}-{sample_count}.png"))
            sample_count += 1
            sample_count_refine = sample_count -1 
      
            refine_rusult = refine_image_with_cot(
                            image=sample,
                            prompt=prompt,
                            think_text=None,
                            cfg_text_scale=4.0, 
            ...
            refine_image = refine_rusult[0].crop(refine_rusult[0].getbbox())
            refine_image.save(os.path.join(sample_path_refine, f"{sample_id}-{sample_count_refine}.png"))
