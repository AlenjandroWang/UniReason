...
class ThinkT2IIterableDataset(InterleavedBaseIterableDataset, DistributedIterableDataset):
    def __init__(
        self, 
        dataset_name, 
        transform, 
    ...
    def parse_row(self, json_line):
        ...
        wrapped_text = f"<think>{thinking.strip()}</think>"
        
        data = self._add_text(data, wrapped_text, need_loss=True, enable_cfg=True)
        
        image = self.load_image_safely(data_item, 'image_path')
