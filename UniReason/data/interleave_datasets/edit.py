...
class TI2ISFTIterableDataset(InterleavedBaseIterableDataset, DistributedIterableDataset):
    def __init__(
        self, 
        dataset_name, 
        transform, 
        ...
        self.image_prefix_dir = image_prefix_dir
    ...
    def parse_row(self, json_line):
        ...
        prompt = data_item.get('instruction', '')
        prompt = f'{prompt}'
        ...
        input_image = self.load_image_safely(data_item, 'input_image')
        output_image = self.load_image_safely(data_item, 'output_image')
        if input_image is not None and output_image  is not None:
            ...
            print(f"Skipping sample due to missing image: {prompt}")
            return {}
